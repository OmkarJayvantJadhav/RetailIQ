...
def generate_clean_name():
    """Ensure names are complete and don't contain titles"""
    while True:
        first = fake.first_name()
        last = fake.last_name()
        if len(first) > 2 and len(last) > 2 and "." not in first and "." not in last:
            return first, last
...
for i in range(NUM_CUSTOMERS):
    first_name, last_name = generate_clean_name()
    selected_state = random.choice(state_names)
    selected_city = random.choice(state_city_map[selected_state])
...
for i in range(NUM_ORDERS):
    ...
    ship_state = random.choice(state_names)
    ship_city = random.choice(state_city_map[ship_state])
